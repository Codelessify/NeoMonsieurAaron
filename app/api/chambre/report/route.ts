...
import { generateChambreReport } from "@/lib/chambre";
  ...
  try {
    ...
    if (body.user_id) {
      try {
        ...
        console.error("[chambre-report] failed to load user context language", err);
      }
    }

    const report = await generateChambreReport(
      body.messages,
      body.inventory,
      contextLanguage
    );
    ...
    console.error("[chambre-report]", err);
